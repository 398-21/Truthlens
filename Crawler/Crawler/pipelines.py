...
class DraftPipeline: 
    ...
    def aggregate_articles_info(self, articles):
        ...
        for article in articles:
            ...
            img_urls = article.get('img', 'No Image URL')  # Assuming 'img' holds the URLs as a comma-separated string
            aggregated_imgUrls += f"{img_urls}\n\n"

        return aggregated_info, aggregated_imgUrls

    def draft_article_with_gpt(self, aggregated_content, aggregated_imgUrls):
        """Use OpenAI's GPT to draft an article based on aggregated content."""

        try:
            # Step 1: Classify the content to check if it's related to Central Asia, Business, or Tech
            prompt_classification = (
                "Based on the following content, first determine if the topic is related to Central Asia. "
                "If the content is related to Central Asia and also touches on Business or Technology, respond with either 'Business' or 'Technology' "
                "depending on the context. If the content is only about Central Asia without any focus on Business or Technology, respond with 'Central Asia'. "
                "If the content is not related to Central Asia at all, respond with 'Other'.\n\n"
                f"Content:\n{aggregated_content}\n\n"
            )


            # Generate classification
            classification_completion = openai.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[{"role": "user", "content": prompt_classification}]
            )
            category = classification_completion.choices[0].message.content.strip().lower()

            # Skip drafting if the content is unrelated
            if category not in ["central asia", "business", "technology"]:
                logging.info("The content is unrelated to the specified categories (Central Asia, Business, Technology). Skipping the drafting process.")
                return None, None, None

            # Proceed with article drafting prompts if content is relevant
            prompt_title = (
                "As a professional content writer, create a main title for a news article based on the following content. "
                "The title should be concise, attention-grabbing, and informative, ideal for an online audience.\n\n"
                f"Content:\n{aggregated_content}\n\n"
                "Output only the title as plain text without any HTML tags or additional formatting."
            )

            prompt_subheaders = (
                "Create engaging subheaders that divide the following content into logical sections. "
                "The subheaders should be informative and improve readability. Provide each subheader as plain text only, "
                "without any HTML tags such as <h2>.\n\n"
                f"Content:\n{aggregated_content}\n\n"
            )

            prompt_content = (
                "As a professional content writer, draft a well-organized news article based on the following content. "
                "The article should be at least 500 words long, divided into sections by the provided subheaders, and include "
                "paragraphs as plain text only, without any HTML tags like <p>.\n\n"
                f"Content:\n{aggregated_content}\n\n"
                "Subheaders:\n[Add previously generated subheaders here]\n\n"
            )

            # Step 3: Generate each part separately
            # Main title generation
            title_completion = openai.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[{"role": "user", "content": prompt_title}]
            )
            drafted_header = title_completion.choices[0].message.content.strip()

            # Subheader generation
            subheaders_completion = openai.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[{"role": "user", "content": prompt_subheaders}]
            )
            drafted_subheader = subheaders_completion.choices[0].message.content.strip()

            # Content generation with subheaders incorporated
            prompt_content_with_subheaders = prompt_content.replace("[Add previously generated subheaders here]", drafted_subheader)
            content_completion = openai.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[{"role": "user", "content": prompt_content_with_subheaders}]
            )
            drafted_content = content_completion.choices[0].message.content.strip()

            # Step 4: Clean up any unwanted completion artifacts or extraneous symbols
            drafted_header = re.sub(r'[\u201c\u201d"]', '', drafted_header)  # Removes weird quotation marks
            drafted_subheader = re.sub(r'[\u201c\u201d"]', '', drafted_subheader)
            drafted_content = re.sub(r'[\u201c\u201d"]', '', drafted_content)
            drafted_content = re.sub(r'[`<>]+', '', drafted_content)  # Remove unwanted punctuation and symbols

            # Step 5: Format HTML with inline CSS for readability
            drafted_header = f'<b>{drafted_header}</b>'
            drafted_subheader = f'<h2>{drafted_subheader}</h2>'

            # Step 6: Append image URLs if provided
            drafted_content += "\n\nImage URL: " + aggregated_imgUrls
            
            drafted_content = f"Category: {category.title()}\n\n" + drafted_content
    ...
    def close(self, items):
        ...
        with open('drafted_articles.txt', 'w', encoding='utf-8') as file:
            for group_id, articles in grouped_articles.items():
                try:
                    aggregated_content, aggregated_imgUrls= self.aggregate_articles_info(articles)
                    
                    drafted_header, drafted_subheader, drafted_content = self.draft_article_with_gpt(aggregated_content, aggregated_imgUrls)
                    ...
                    # Log the error and continue to the next group
                    logging.error(f"Error while processing Group {group_id}: {e}")
                    continue 
    ...
    def insert_into_db(self, header, subheader, content):
        ...
        default_post_status = 'draft'  # or 'publish' if the posts are ready to go live
