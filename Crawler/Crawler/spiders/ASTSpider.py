    ...
    def parse_news_content(self, response):
        ...
        try:
            ...
            if date_obj.date() == datetime.now().date():
                ...
                img_urls = response.css('div.post div.wp-caption img::attr(src)').getall()
                news_item['img'] = ', '.join(img_urls) 
                # news_item['img'] = response.css('div.post div.wp-caption.aligncenter img::attr(src)').getall()
                img_captions = response.css('div.post p.wp-caption-text::text').getall()
                news_item['img_caption'] = ', '.join(img_captions)
